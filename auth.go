...
func (m *Monitor) validateReturnURL(returnURL string) string {
...
func (m *Monitor) AuthMiddleware(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil || !m.sessionManager.ValidateSession(cookie.Value) {
			...
			safeReturnURL := m.validateReturnURL(returnURL)
...
func (m *Monitor) handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		// Check if locked out
		if m.sessionManager.IsLockedOut(ip) {
			// Validate return URL
			safeReturnURL := m.validateReturnURL(r.FormValue("return"))
			...
			m.templates.ExecuteTemplate(w, "login.html", data)
			return
		}

		// Verify password
		password := r.FormValue("password")
		valid, err := VerifyArgon2Hash(password, m.config.PasswordHash)
		if err != nil || !valid {
			m.sessionManager.RecordFailedLogin(ip)
		...
		m.sessionManager.ResetLoginAttempts(ip)

		// Create session
		token, err := m.sessionManager.CreateSession()
		if err != nil {
